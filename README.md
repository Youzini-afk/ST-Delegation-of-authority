...
- **默认访问策略**：当前所有资源的系统默认值都是 `granted`，新装扩展开箱即用；管理员可再收紧为 `prompt` / `denied` / `blocked`
...
- **系统默认值**：当前所有资源的内置默认策略都是 `granted`，所以大多数扩展安装后可直接使用。
- **管理员策略**：支持全局默认策略和按扩展覆盖策略；`granted` / `denied` / `blocked` 会优先于用户 grant，`prompt` 则表示“没有用户 grant 时需要询问”。
...
declared permissions gate
  > admin extension override
  > admin default (granted / denied / blocked)
  > persistent grant
  > session grant
  > admin default (prompt)
  > system default (granted)
```

这意味着：

- 没有管理员收紧规则时，大多数请求会被系统默认直接放行。
- 管理员把某类资源设成 `prompt` 时，用户仍然可以通过 `allow-once` / `allow-session` / `allow-always` 继续授权。
- 管理员把某类资源设成 `denied` 或 `blocked` 时，会强于用户自己点过的允许。
