...
- **Slice L limits 合同升级**
  - `/probe` 与 session 返回现在都带 `limits`
  - `effectiveInlineThresholdBytes` 按操作暴露 inline-vs-transfer 决策阈值，并带 `runtime` / `policy` source
  - `effectiveTransferMaxBytes` 按操作暴露 transfer ceiling，区分 `storageBlob*`、`privateFile*`、`httpFetch*`
  - extension-scoped limits policy 已进入 control policies document，可对 inline threshold 做策略下压
...
- **管理员运维面板与高层 artifact**
  - Security Center 的 `Updates` 页已扩展为管理员运维面板
  - 新增 `Usage Summary`、portable package import/export、operation 持久跟踪与 diagnostic archive
  - portable package 默认导出为 `.authoritypkg.zip` 多文件归档，并继续兼容导入旧 `.json.gz`
...
- **管理员 limits policy**：支持按扩展声明 inline threshold override，session 返回 effective limits。
- **审计记录**：记录 permission、usage、warning、error 四类活动。
- **host/path escape 防护**：SQL 数据库路径、Blob 路径和 `fs.private` 私有目录都由服务端映射并做路径逃逸检查。
- **资源限制**：`/probe` 暴露公开 adapter limits 与 effective limits；session 暴露 extension-scoped effective limits；core health 暴露内部 hard ceiling。
...
- `effectiveInlineThresholdBytes`
  - 是 extension-scoped 的 inline-vs-transfer 决策值
  - 当前 source 可能为 `runtime` 或 `policy`

- `effectiveTransferMaxBytes`
  - 是按操作暴露的 transfer ceiling
  - 当前 source 为 `runtime`
...
- KV 命名空间隔离
- Blob 读写与删除
- session-scoped effective inline thresholds / transfer ceilings 合同
- transfer purpose 与按操作 transfer ceiling 回归
- 内建 delay 任务创建、完成、取消
