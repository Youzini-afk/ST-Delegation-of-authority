...
function renderTimeline(snapshot: AgentSessionSnapshot): string {
    const ref = snapshot.refs.find(item => item.name === 'main') ?? snapshot.refs[0];
...
function renderInspector(
    ...
    selectedProfile: AgentLlmProfile | null,
    disabled: string,
): string {
...
function renderSettingsInspector(state: AgentWorkbenchState, selectedProfile: AgentLlmProfile | null, disabled: string): string {
    const snapshot = state.selectedSession;
    return `
...
function renderWorkspaceStudio(
    state: AgentWorkbenchState,
...
function modeOptions(selected: 'plan' | 'ask' | 'auto'): string {
    return ([['ask', 'ask · 变更前审批'], ['plan', 'plan · 只规划'], ['auto', 'auto · 按策略执行']] as const)
