...
export function getActiveAgentSessionRun(snapshot: AgentSessionSnapshot | null): AgentSessionRun | null {
...
export function isActiveAgentSession(snapshot: AgentSessionSnapshot | null): boolean {
...
function renderSessionMenu(state: AgentWorkbenchState, snapshot: AgentSessionSnapshot, disabled: string): string {
    return `
...
function renderSessionHistory(state: AgentWorkbenchState, disabled: string): string {
    return `
...
function renderTimeline(snapshot: AgentSessionSnapshot, disabled: string): string {
    const ref = snapshot.refs.find(item => item.name === 'main') ?? snapshot.refs[0];
...
function renderTimelineEntry(entry: AgentSessionConversationEntry, snapshot: AgentSessionSnapshot): string {
...
function renderInspector(
    ...
    selectedWorkspace: AgentWorkbenchState['workspaces'][number] | null,
    disabled: string,
): string {
    const tabs: Array<[AgentWorkbenchState['inspectorTab'], string]> = [
...
function renderActivityInspector(state: AgentWorkbenchState, disabled: string): string {
    const snapshot = state.selectedSession;
...
function renderPendingApprovals(snapshot: AgentSessionSnapshot, disabled: string, extraClass = ''): string {
...
function renderChangesInspector(
    state: AgentWorkbenchState,
    selected: AgentWorkbenchState['workspaces'][number] | null,
    disabled: string,
): string {
...
function renderSessionStatus(status: AgentSessionStatus): string {
...
function sessionStatusLabel(status: AgentSessionStatus): string {
    return ({
    ...
    } satisfies Record<AgentSessionStatus, string>)[status];
}

function renderInvocation(invocation: AgentSessionToolInvocation): string {
...
function toolStatusLabel(status: AgentSessionToolInvocation['status']): string {
    return ({
        pending: '等待执行', waiting_approval: '等待审批', claimed: '执行中', completed: '完成', failed: '失败',
        cancelled: '取消', outcome_unknown: '结果未知', timed_out: '超时',
    } satisfies Record<AgentSessionToolInvocation['status'], string>)[status];
}

function queueKindLabel(kind: 'steer' | 'follow_up' | 'next_run'): string {
    return kind === 'steer' ? '调整' : kind === 'follow_up' ? '继续' : '下一轮';
}

function modeLabel(mode: 'plan' | 'ask' | 'auto'): string {
    return mode === 'plan' ? '只规划' : mode === 'auto' ? '按策略自动执行' : '审批后执行';
}

function modeOptions(selected: 'plan' | 'ask' | 'auto'): string {
    return ([['ask', '审批后执行'], ['plan', '只规划，不修改'], ['auto', '按全局策略自动执行']] as const)
...
function previewValue(value: unknown): string {
    try {
        return previewText(JSON.stringify(value, (key, item) => (
            key && SENSITIVE_FIELD_NAMES.has(key.replace(/[^a-z0-9]/gi, '').toLowerCase()) ? '[REDACTED]' : item
        ), 2), 20_000);
    } catch {
        return '[无法序列化]';
    }
