...
import type { WorkspaceFileDiffLoadState } from './types.js';

export type WorkspaceDiffViewScope = 'working' | 'history';

export interface WorkspaceDiffViewContext {
    action: 'agent-file-diff' | 'system-file-diff';
    scope: WorkspaceDiffViewScope;
    workspaceId: string;
    from: string | null;
...
export function workspaceFileDiffKey(
    workspaceId: string,
    from: string | null,
    to: string | null | 'working',
    path: string,
...
export function renderWorkspaceDiffEntries(
    entries: WorkspaceDiffEntry[],
    context: WorkspaceDiffViewContext,
): string {
    return `<div class="authority-file-diff-list">${entries.map(entry => renderEntry(entry, context)).join('')}</div>`;
}

function renderEntry(entry: WorkspaceDiffEntry, context: WorkspaceDiffViewContext): string {
...
function renderFileDiff(diff: WorkspaceFileDiffResponse): string {
    if (diff.kind === 'binary') {
...
function textMetadataNotice(diff: WorkspaceFileDiffResponse): string | null {
    const metadata = diff.textMetadata;
    if (diff.status !== 'modified' || !metadata) return null;
...
function lineEndingLabel(value: NonNullable<WorkspaceFileDiffResponse['textMetadata']>['before']['lineEnding']): string {
    return ({ none: '无', lf: 'LF', crlf: 'CRLF', cr: 'CR', mixed: '混合' } as const)[value];
}

function statusMark(status: WorkspaceDiffEntry['status']): string {
    return ({ added: 'A', modified: 'M', deleted: 'D', type_changed: 'T' } as const)[status];
}

function unavailableLabel(reason: WorkspaceFileDiffResponse['reason']): string {
    if (reason === 'file_too_large') return '文件超过 512 KiB，未加载内容差异。';
    if (reason === 'diff_too_complex') return '差异规模过大，无法在控制台内安全展开。';
    return '该路径类型不支持文本差异。';
}
