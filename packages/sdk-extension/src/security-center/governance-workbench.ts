...
import type { ExtensionStorageSummary, SecurityCenterState } from './types.js';
import { buildOverviewModel, getDatabaseGroupSummaries } from './view-models.js';

export interface ExtensionDirectoryMarkup {
    count: number;
    html: string;
}

export function renderExtensionDirectory(state: SecurityCenterState): ExtensionDirectoryMarkup {
...
export function renderGovernanceOverview(state: SecurityCenterState): string {
...
export function renderExtensionDossier(state: SecurityCenterState): string {
...
export function renderDataAssets(state: SecurityCenterState): string {
...
export function renderAuditWorkspace(state: SecurityCenterState): string {
...
export function renderPolicyWorkbench(state: SecurityCenterState): string {
    if (!state.isAdmin) {
    ...
    const defaultCounts = STATUS_OPTIONS.reduce<Record<PermissionStatus, number>>((counts, status) => {
...
export function renderPolicyOverrideRow(entry?: AuthorityPolicyEntry): string {
    return `
...
function renderDeclaredCapabilityRows(declared: DeclaredPermissions): string {
...
function renderCompactStorageSummary(storage: ExtensionStorageSummary): string {
    const items = [
