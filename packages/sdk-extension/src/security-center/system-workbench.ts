                ...
                <div class="authority-system-view__scroll">
                    ...
                    <section class="authority-runtime-section">
                        ...
                        <div class="authority-component-table">
                            ...
                            <div><strong>服务端插件</strong><code>${escapeHtml(probe?.pluginVersion ?? MISSING_TEXT)}</code><span>${escapeHtml(probe ? getInstallStatusLabel(probe.installStatus) : MISSING_TEXT)}</span><button type="button" class="authority-text-button" data-action="admin-update" data-update-action="git-pull" ${disabled}>更新</button></div>
                            <div><strong>前端 SDK</strong><code>${escapeHtml(probe?.sdkDeployedVersion ?? MISSING_TEXT)}</code><span class="${probe?.sdkDeployedVersion === probe?.sdkBundledVersion ? 'authority-status-text--granted' : 'authority-status-text--prompt'}">${probe?.sdkDeployedVersion === probe?.sdkBundledVersion ? '版本一致' : '需要部署'}</span><button type="button" class="authority-text-button" data-action="admin-update" data-update-action="redeploy-sdk" ${disabled}>重新部署</button></div>
                            <div><strong>Authority Core</strong><code>${escapeHtml(probe?.coreArtifactPlatform ?? MISSING_TEXT)}</code><span class="${probe?.coreVerified ? 'authority-status-text--granted' : 'authority-status-text--denied'}">${probe?.coreVerified ? 'Hash 匹配' : '校验未通过'}</span><span></span></div>
                        </div>
                    </section>
...
function detailFact(label: string, value: string): string {
    return `<span><small>${escapeHtml(label)}</small><code>${escapeHtml(value)}</code></span>`;
}
