...
export function bootstrapHostEventRecorder() {
    if (started || typeof globalThis.addEventListener !== 'function')
        return;
    started = true;
    globalThis.addEventListener('authority:chat-committed', receiveCommitEvent);
    globalThis.addEventListener('authority:chat-loaded', receiveCommitEvent);
    setTimeout(() => {
        const bridge = globalThis.STAuthorityHostBridge;
        enqueue(bridge?.getLatestCommit?.());
    }, 0);
}
function receiveCommitEvent(event) {
...
function scheduleRetry() {
    if (retryTimer !== null || pending.size === 0)
        return;
...
function normalizeCommit(value) {
    ...
    if (!eventId || !transactionId || !conversationId || !branchId || !committedAt)
        return null;
    if (baseRevision === null || revision !== baseRevision + 1)
        return null;
    ...
    return {
        ...
        ...(Array.isArray(value.changes) ? { changes: value.changes.map(normalizeChange).filter(Boolean) } : {}),
        committedAt,
    };
}
...
//# sourceMappingURL=host-event-recorder.js.map