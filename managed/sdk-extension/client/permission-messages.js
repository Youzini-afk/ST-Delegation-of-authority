export function getPermissionFailureMessage(displayName, resource, target, decision) {
...
export function getPermissionEvaluationMessage(displayName, resource, target, decision) {
    if (decision === 'granted') {
        const resourceName = getPermissionResourceLabel(resource);
        const resourceLabel = target && target !== '*' ? `${resourceName} (${target})` : resourceName;
...
export function getAuthorityPermissionErrorCode(decision) {
    if (decision === 'denied') {
...
export function getPermissionResourceLabel(resource) {
    switch (resource) {
        case 'storage.kv':
            return 'KV 存储';
        case 'storage.blob':
...
//# sourceMappingURL=permission-messages.js.map