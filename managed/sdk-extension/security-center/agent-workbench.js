...
const ACTIVE_RUN_STATUSES = new Set(['queued', 'running', 'waiting_approval', 'waiting_browser_tool']);
const SENSITIVE_FIELD_NAMES = new Set([
    'apikey', 'authorization', 'accesstoken', 'refreshtoken', 'token', 'secret',
    'clientsecret', 'password', 'passwd', 'cookie', 'setcookie',
]);
export function isActiveAgentRun(status) {
...
export function renderAgentRunDetail(detail, busy = false) {
    if (!detail) {
...
function renderRunLauncher(state, disabled) {
    return `
...
function renderRunHistory(state, disabled) {
    const statuses = [
...
function renderWorkspaceStudio(state, selected, disabled) {
...
function renderCommitRow(commit, head, disabled) {
...
function previewValue(value) {
    try {
        return previewText(JSON.stringify(value, (key, item) => (key && SENSITIVE_FIELD_NAMES.has(key.replace(/[^a-z0-9]/gi, '').toLowerCase())
            ? '[REDACTED]'
            : item), 2), 20_000);
    }
    catch {
